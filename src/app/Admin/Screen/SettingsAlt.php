...
class SettingsAlt extends Screen
{
    public function init()
    {
        if ( $code = $_GET['code'] ?? null ) {
            ...
            {
                $admin_url = admin_url('admin.php?page=zorgportal-settings2');
                echo "<script>history.replaceState({}, {}, '{$admin_url}')</script>\n";
            });

            if ( ($res['access_token'] ?? '') && ($res['refresh_token'] ?? '') ) {
                $res['_expires'] = time() + intval($res['expires_in'] ?? 0);
                update_option('zp_alt_exactonline_auth_tokens', $res);
    ...
    public function render()
    {
        if ( ! isset( $_POST['oauth_redirect'] ) ) {
            $_POST['client_id'] = get_option('zorgportal_alt_exact_client_id') ?: '';
            $_POST['client_secret'] = get_option('zorgportal_alt_exact_client_secret') ?: '';
            $_POST['webhook_secret'] = get_option('zorgportal_alt_exact_webhook_secret') ?: '';
        }

        return $this->renderTemplate('settings.php', [
            'nonce' => wp_create_nonce('zorgportal'),
            'tokens' => $tokens=get_option('zp_alt_exactonline_auth_tokens'),
            'division' => get_option('zp_alt_exactonline_current_division'),
            'divisions' => get_option('zp_alt_exactonline_divisions'),
            ...
            'api_usage_minute' => App::getCounter(sprintf('%s/eo-usage/%s', $client_id=get_option('zorgportal_alt_exact_client_id'), date('H-i'))),
    ...
    private function oauthRedirect()
    {
        ...
        update_option('zorgportal_alt_exact_client_id', $client_id);
        update_option('zorgportal_alt_exact_client_secret', $client_secret);
        update_option('zorgportal_alt_exact_webhook_secret', $webhook_secret);

        /*wp_redirect(add_query_arg([
            'client_id' => $client_id,
            'redirect_uri' => urlencode( admin_url('admin.php?page=zorgportal-settings2') ),
            'response_type' => 'code',
            'force_login' => '0 ',
        ], 'https://start.exactonline.nl/api/oauth2/auth'));
        exit;*/
        $connection = new Connection();
        $connection->setRedirectUrl(admin_url('admin.php?page=zorgportal-settings2'));
    ...
    {
        if ( ! $client_id = get_option('zorgportal_alt_exact_client_id') )
            return null;

        if ( ! $client_secret = get_option('zorgportal_alt_exact_client_secret') )
            return null;
        ...
        $res = wp_remote_post($url='https://start.exactonline.nl/api/oauth2/token', $params=[
            ...
            'body' => sprintf('%s=%s&redirect_uri=%s&grant_type=%s&client_id=%s&client_secret=%s',
                ...
                urlencode( admin_url('admin.php?page=zorgportal-settings2') ),
                $grant_type,
                $client_id,
                $client_secret
            ),
    ...
    public static function refreshTokensCron() : bool
    {
        ...
        if ( ($res['access_token'] ?? '') && ($res['refresh_token'] ?? '') ) {
            ...
            delete_option('zp_alt_exactonline_auth_tokens');
            return false;
        }
    }
    ...
    {
        $tokens = get_option('zp_alt_exactonline_auth_tokens');
        ...
        update_option('zp_alt_exactonline_current_division', $division);
    ...
    {
        if ( ! $division = get_option('zp_alt_exactonline_current_division') )
            return $this->error( __('Error occurred: bad request.', 'zorgportal') );
        ...
        update_option('zp_alt_exactonline_divisions', $divisions);
    ...
    {
        if ( delete_option('zp_alt_exactonline_auth_tokens') ) {
