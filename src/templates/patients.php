...
<div class="wrap">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;margin-bottom:.55rem">
        <form style="display:flex;align-items:center;justify-content:space-between;flex:1">
            <h1 style="padding:9px 9px 9px 0"><?php _e('Zorgportal &lsaquo; Patients', 'zorgportal'); ?></h1>
            <a href="admin.php?page=zorgportal-new-patient" class="page-title-action" style="position:static"><?php _e('Add New', 'zorgportal'); ?></a>
            ...
            <select name="address" style="margin-left:4px;max-width:150px">
                <option value=""><?php esc_attr_e('&mdash; address &mdash;', 'zorgportal'); ?></option>
            ...
            <select name="status" style="margin-left:2px">
                <option value=""><?php esc_attr_e('&mdash; status &mdash;', 'zorgportal'); ?></option>
                <?php foreach ( [
            ...
            <select name="insurer" style="margin-left:4px;max-width:150px">
                <option value=""><?php esc_attr_e('&mdash; insurer &mdash;', 'zorgportal'); ?></option>
                <?php foreach ( $insurers as $insurer ) : ?>
                    <option <?php selected(esc_attr($insurer) == ($_GET['insurer'] ?? '')); ?>><?php echo esc_attr($insurer); ?></option>
                <?php endforeach; ?>
            </select>

            <select name="policy" style="margin-left:4px;max-width:150px">
                <option value=""><?php esc_attr_e('&mdash; policy &mdash;', 'zorgportal'); ?></option>
        ...
        <form method="post" action="admin.php?page=zorgportal-patients" onsubmit="return confirm('<?php esc_attr_e('Are you sure?', 'zorgportal'); ?>')" style="margin-left:4px">
